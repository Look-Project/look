...
@MultipartConfig
@WebServlet("/controller/WorkwearEditController")
public class WorkwearEditController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        WorkwearBoardResponse wbr = null;
       
		wbr = wdao.getDetailBoard(num);
		request.setAttribute("onecondto", wbr);
		...
		if (loginMember == null) {
			...
			request.getRequestDispatcher(WORKWEAR_EDIT_VIEW).forward(request, response);
		}
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	WorkwearEditRequest wer = new WorkwearEditRequest();
		...
		if(contentType != null && contentType.toLowerCase().startsWith("multipart/")) {
			...
			for(Part part : parts) {
				...
				if(part.getHeader("Content-Disposition").contains("filename=")) {
					...
					String uploadPath = WORKWEAR_UPLOAD_PATH + todayDate;
					...
					if(part.getSubmittedFileName() == null || part.getSubmittedFileName().isBlank()) {
		                  response.sendRedirect(request.getContextPath() + WORKWEAR_EDIT_VIEW);
		               }
					...
					if(part.getSize() > 0) {
						...
						part.delete();
						wer.setImgSrc(uploadPath);
						wer.setImgName(newFileName);
    	...
    	WorkwearService wws = new WorkwearService();
        ...
        // DTO에 저장
        wer.setBoardId(boardId);
        wer.setTitle(title);
        wer.setContents(contents);
